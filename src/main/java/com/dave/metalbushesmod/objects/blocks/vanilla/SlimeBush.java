...
public class SlimeBush extends SweetBerryBushBlock {

    public SlimeBush(Properties properties) {
        super(properties);
    }

    @Override
    public ItemStack getCloneItemStack(BlockGetter worldIn, BlockPos pos, BlockState state) {
        return new ItemStack(BlockInit.SLIME_BUSH.get());
    }
    ...
    public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!flag && player.getItemInHand(handIn).is(Items.BONE_MEAL)) {
            ...
            popResource(worldIn, pos, new ItemStack(ItemInit.SLIME_CHUNK.get(), 1));
            ...
            popResource(worldIn, pos, new ItemStack(ItemInit.SLIME_CHUNK.get(), j));
            worldIn.playSound((Player) null, pos, SoundEvents.SWEET_BERRY_BUSH_PICK_BERRIES, SoundSource.BLOCKS, 1.0f, 0.8F + worldIn.random.nextFloat() * 0.4F);
            worldIn.setBlock(pos, state.setValue(AGE, Integer.valueOf(1)), 2);

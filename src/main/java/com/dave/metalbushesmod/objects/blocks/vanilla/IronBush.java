...
public class IronBush extends SweetBerryBushBlock {
    ...
    public IronBush(Properties properties/* , Item materialIn */) {
        super(properties);
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if(!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
            ...
            spawnAsEntity(worldIn, pos, new ItemStack(Items.IRON_NUGGET, 1));
            worldIn.playSound((PlayerEntity) null, pos, SoundEvents.ITEM_SWEET_BERRIES_PICK_FROM_BUSH, SoundCategory.BLOCKS, 1.0f, 0.8F + worldIn.rand.nextFloat() * 0.4F);
            worldIn.setBlockState(pos, state.with(AGE, Integer.valueOf(1)), 2);

            return ActionResultType.SUCCESS;
        } else if (i == 3) {
            spawnAsEntity(worldIn, pos, new ItemStack(Items.IRON_NUGGET, 2));
            worldIn.playSound((PlayerEntity) null, pos, SoundEvents.ITEM_SWEET_BERRIES_PICK_FROM_BUSH, SoundCategory.BLOCKS, 1.0f, 0.8F + worldIn.rand.nextFloat() * 0.4F);
            worldIn.setBlockState(pos, state.with(AGE, Integer.valueOf(1)), 2);
